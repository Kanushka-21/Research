...
Two live classification sessions were logged, either side of the imaging-conditions correction of Section~\ref{sec:annotation_qc}; their class-distribution totals are reproduced from Table~\ref{tab:liveclass} in Table~\ref{tab:appendixB_live}. Photographic evidence was retained from both sessions. Neither session recorded a per-fruit ground-truth label, so, as stated in Section~\ref{sec:r_live}, no accuracy percentage can be derived from this record.
...
\textbf{Mechanical transport (completed).} A bench trial confirmed smooth, stable belt transport of fresh fruit with no vision or sorting logic active (Section~\ref{sec:r_sorting}). \textbf{[INFORMATION REQUIRED]} Fruit count and belt speed for this trial were not recorded at the time and could not be located.

\textbf{Closed-loop end-to-end sorting (not carried out).} No trial converting live classifications into gate commands and checking physical diversion against known fruit has been run on the assembled hardware. No per-class diversion count or failure-mode breakdown can therefore be reported for this experiment (Section~\ref{sec:r_sorting}); T-06, T-07 and T-08 in Table~\ref{tab:testoutcomes} are correspondingly marked "not yet tested." The firmware and command protocol needed for this trial (Appendix~C) were confirmed to build without error and to respond correctly to manually issued test commands; what remains untested is the firmware driving actual gate diversions from live classifications on a moving belt with real fruit.
...
\textbf{[INFORMATION REQUIRED]} No raw day-by-day observation record for the 100-tomato, 31-day trial described in Section~\ref{sec:m_shelflife} was found, and no recorded storage temperature or relative humidity for that trial was found either. This is the same gap already identified in Section~\ref{sec:r_shelflife}: the values above match \citet{abekoon2024}'s published Padma stage durations exactly once mapped through the four-class correspondence of Table~\ref{tab:merge}, which is more consistent with the published figures having been carried into the project's records than with an independent 100-fruit replication. Until the underlying raw record is located, or the 31-day trial is repeated and logged, this table should continue to carry that caveat rather than be presented as confirmed independent measurement.

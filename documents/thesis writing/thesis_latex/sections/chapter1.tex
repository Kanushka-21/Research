...
The economic argument rests on the loss that objective grading avoids. Replacing repeated manual inspection with a single automated pass reduces handling damage and removes the operator-to-operator variation that lets overripe fruit into packed batches \citep{abekoon2024}.
...
\begin{enumerate}
    ...
    \item To train and evaluate a YOLOv8-medium object detector on this dataset against a held-out test split, and to compare it under identical data conditions with a YOLO26-medium model in order to justify the deployed architecture on evidence rather than assertion.
...
\begin{enumerate}
    ...
    \item A trained detector for Padma ripening stage and defect condition, with performance reported on a held-out test split and against a comparison architecture.

...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\linewidth]{figures/Tomato Intro .jpeg}
\caption{Tomato cultivation areas in the Badulla District and major local cultivars.}
\label{fig:tomato-introduction}
\end{figure}
...
\begin{enumerate}
    \item How accurately can a single-stage object detector localise and classify Padma fruit into four ripening stages and a defect class from a purpose-built, cultivar-specific dataset, and which architecture within that family performs best on this task?
...
Second, it does not treat detection accuracy as just an offline number. Instead, it builds and tests a real, working sorting machine that uses the detector's output to physically sort fruit, triggered by sensors. Reporting this failure, and how it was corrected, is itself a useful contribution, since this type of mistake is invisible in ordinary offline test scores and only shows up when the system is tested in real, live conditions.
...
\begin{enumerate}
    ...
    \item To train and evaluate a YOLOv8 object detector on this dataset against a held-out test split, and to compare it under identical data conditions with a YOLO26-medium model in order to justify the deployed architecture on evidence rather than assertion.
    ...
    \item To design and construct a conveyor prototype controlled by an ESP32 microcontroller, in which gate actuation is released by an \acrfull{IR} presence sensor at each gate rather than by an elapsed time computed from belt speed.
...
\begin{enumerate}
    ...
    \item A trained model for Padma ripening stage and defect condition, with performance reported on a held-out test split and against a comparison architecture.
